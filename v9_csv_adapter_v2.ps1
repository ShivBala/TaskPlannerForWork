...
function Read-V9ConfigFile {
    ...
    try {
        ...
        # First, extract the TICKETS section and parse it properly using Import-Csv
        # This handles multi-line CSV fields correctly
        $ticketsSectionStart = $content.IndexOf("SECTION,TICKETS")
        $ticketsData = @()
        if ($ticketsSectionStart -ge 0) {
            $ticketsContent = $content.Substring($ticketsSectionStart)
            # Skip "SECTION,TICKETS" line
            $ticketsContent = $ticketsContent.Substring($ticketsContent.IndexOf("`n") + 1)
            # Extract just the tickets CSV data (stop at next SECTION or end of file)
            if ($ticketsContent -match '(?s)^(.*?)(?:SECTION,|$)') {
                $ticketsCsv = $Matches[1].Trim()
                if ($ticketsCsv) {
                    # Use Import-Csv which properly handles multi-line quoted fields
                    $ticketsData = $ticketsCsv | ConvertFrom-Csv
                }
            }
        }
        ...
        for ($i = 0; $i -lt $lines.Count; $i++) {
            ...
            switch ($currentSection) {
                ...
                'TICKETS' {
                    # Skip - tickets are already parsed using ConvertFrom-Csv at the beginning
                    # This ensures multi-line CSV fields are handled correctly
                    continue
                }
            }
        }
        
        # Process the pre-parsed tickets data
        foreach ($ticket in $ticketsData) {
            ...
            # Handle V10 vs V9 column name difference for Details fields
            # V10 uses "Details Description" (no colon), V9 uses "Details: Description" (with colon)
            if ($ticket.PSObject.Properties.Name -contains 'Details Description') {
                $ticketObj | Add-Member -NotePropertyName 'DetailsDescription' -NotePropertyValue $ticket.'Details Description'
                $ticketObj | Add-Member -NotePropertyName 'DetailsPositives' -NotePropertyValue $ticket.'Details Positives'
                $ticketObj | Add-Member -NotePropertyName 'DetailsNegatives' -NotePropertyValue $ticket.'Details Negatives'
            } elseif ($ticket.PSObject.Properties.Name -contains 'Details: Description') {
                $ticketObj | Add-Member -NotePropertyName 'DetailsDescription' -NotePropertyValue $ticket.'Details: Description'
                $ticketObj | Add-Member -NotePropertyName 'DetailsPositives' -NotePropertyValue $ticket.'Details: Positives'
                $ticketObj | Add-Member -NotePropertyName 'DetailsNegatives' -NotePropertyValue $ticket.'Details: Negatives'
            }
